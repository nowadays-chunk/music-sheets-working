...
const FretboardTable = styled('table')({
  ...
  tableLayout: 'fixed',          // <- prevents resizing
  borderSpacing: 0,
});
...
const TableRow = styled('tr')({
  margin: 0,
  padding: 0,
  height: CELL_SIZE,             // fixed row height
});
...
const Note = styled('div')({
  position: 'absolute',          // <- does not affect cell layout
  top: '50%',
  left: '50%',
...
const getNoteStyle = (note) => {
  if (!note.show) return { backgroundColor: 'transparent' };

  let backgroundColor = 'lightblue';
  if (note.interval === '1' || note.interval === 'root') backgroundColor = 'lightsalmon';
  if (note.interval === '3' || note.interval === 'b3') backgroundColor = 'lightblue';
  if (note.interval === '5') backgroundColor = 'lightblue';
  if (note.interval === '7' || note.interval === 'b7') backgroundColor = 'lightblue';
    ...
    const newRows = [...Array(numStrings)].map((_, i) => (
      <TableRow key={`row-${fretboardIndex}-${i}`}>
        ...
        {fretNumbers.map((fret) => {
          ...
          const thirdNote =
            note.show && (noteIndex === 2 || ['3', 'b3'].includes(note.interval))
              ? 'third-note'
              : '';
          const fifthNote = note.show && (noteIndex === 4 || note.interval === '5') ? 'fifth-note' : '';
          const seventhNote =
            note.show && (noteIndex === 6 || ['7', 'b7'].includes(note.interval))
              ? 'seventh-note'
              : '';
          ...
          return (
            ...
            >
              {/* line always present, behind note */}
              <NoteLine />

              {/* circle only when note.show, but cell size never changes */}
              {note.show && (
                <Note
