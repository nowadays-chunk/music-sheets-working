...
const withPlayback = (WrappedComponent) => {
    return (props) => {
        ...
        const prevChordRef = React.useRef(null);
        ...
        const getChordSignature = () => {
            ...
            return JSON.stringify({
                ...
                // notes may be undefined sometimes, so we default to empty array
        ...
        useEffect(() => {
            ...
            if(selectedFretboard.chordSettings.chord !== '') return;
            if(selectedFretboard.chordSettings.shape !== '') return;
            if(selectedFretboard.chordSettings.notes.length > 0) return;
            
            const newSig = getChordSignature();

            // If signature didn’t change → don't replay
            if (prevSig.current === newSig) return;

            // Update the stored signature
            prevSig.current = newSig;

            // Now chord change is confirmed → PLAY ONCE
            playSelectedNotes();
        ...
        }, [dispatch, selectedFretboard, selectedFretboardIndex]);
        ...
        const playSelectedNotes = async () => {
            ...
            if (choice === 'chord') {
                console.log("playing chord notes once")
